...
export default async function Home() {
  try {
    // First, list all available regions to debug
    const allRegions = await listRegions()
    console.log("All available regions:", allRegions)
    
    // Log all country codes available
    if (allRegions) {
      allRegions.forEach((region) => {
        console.log(`Region: ${region.name}, Countries:`, region.countries?.map(c => c.iso_2))
      })
    }

    const DEFAULT_REGION = process.env.NEXT_PUBLIC_DEFAULT_REGION || "il"
    console.log("Trying to load region with code:", DEFAULT_REGION)
    ...
    // Don't require region for home page - make it optional
    if (!collections) {
